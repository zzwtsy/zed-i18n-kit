...
Depends on: [阶段 1C-A 审核协议与 fail-closed 门禁](../completed/2026-09-02-phase-1c-a-review-gate.md) 已由主 Agent 验收；本工作项仍不得进入 1C-C。
...
- r2 审核失败后废弃该 evidence set，准备新的 `phase-1c-corpus-final-r3` / `phase-1c-unlabeled-final-r3` 身份；只有 HIR 修复和预审核门禁通过后才生成新 blind bundle；
...
- 不得复用失败的 r2 review/audit result，或在 HIR 修复前把 r3 候选身份标记为 `frozen`；
- 不引入阶段 2 persistent inventory、Message ID、Overlay、runtime、目录或源码 rewrite；不修改 `local/zed`。

## 串行验收条件

- [x] 1C-A 已由主 Agent 验收，且 staged patch 边界和 `local/zed` 状态保持不变；
...
- [x] r2 的 266 条 corpus review 与 100 条 holdout audit 已真实执行并准确记录失败，不复用为最终证据；
- [x] `.child(match ...)` element 误报和纯格式控制字面量已由通用规则修复，并有正反回归测试；
- [ ] 7 条 external ownership exclusion leakage 尚未归零；最小 HIR 来源解析是当前阻塞项；
- [ ] r3 全量 review、全新 100 条 holdout audit 和最终冻结尚未执行，不能声称完成。

## 验证证据

| 命令或操作 | 状态 | 证明内容 | 限制 |
| --- | --- | --- | --- |
| 1C-A 验收记录 | passed | A 已由主 Agent 验收；本切片仅获准进行 corpus canonicalization、scanner 收敛和预审核指标准备，独立审核留在 1C-C | 不等于真实 review/audit 已完成 |
...
| `git diff --check`、`git diff --cached --binary \| sha256sum`、`git -C local/zed status --short` | passed；差异无空白错误，暂存区 SHA 保持 `129483a898027a9eecc13d63e647f75e7f506a1d022373b18cd20cd23b296e98`，`local/zed` 无输出 | 变更边界和输入 checkout 未被本轮验证操作污染 | 不证明功能或审核协议已完成 |
| 候选 blind audit bundle 生成 | passed；已生成并重复比较 `phase-1c-unlabeled-final-r2` bundle | 为 1C-C 准备不泄漏预测的审核输入 | 不包含独立审核 result |
| `env UV_CACHE_DIR=/tmp/zed-i18n-kit-uv-cache uv build --out-dir /tmp/zed-i18n-kit-wheel-second-batch` 与 wheel 内容检查 | passed；生成 `zed_i18n_kit-0.1.0` sdist/wheel，wheel 包含最终 freeze policy（与源码字节一致）、6 个 schema、CLI、`cst_canonical.py`、`rust_macros.py` 和 scanner | 构建产物携带阶段 1C 所需持久资源 | 未创建隔离环境执行安装态 CLI；1C-C 仍因独立审核缺失而 blocked |
| r2 独立 corpus review / dispute adjudication | failed；266/266 完成，首次对账 `agreed=246`、`disputed=20`、`missing=0`；全新隔离裁决者复核 20 条，确认协议、身份、用户和诊断来源标签需要修订 | 首次获得不读取旧标签或 scanner prediction 的真实分歧证据 | corpus 改动使 r2 身份失效，必须完整生成 r3 review，不能合并局部结果 |
| r2 独立 holdout audit | failed；100/100 完成，`agreement=85`、`conservative_review=5`、`unsafe_promotion=0`、`corpus_gap=0`、`indeterminate=0`、candidate/excluded mismatch=10 | 证明无 unsafe promotion，但揭示 2 个 scanner 缺陷、2 个 reviewer 对技术名词的误排除和 6 个待裁决语义边界 | r2 audit set 已废弃，不得复用为最终 holdout |
...
| r3 预审核 scan/evaluate | failed gate；scan 为 2695 occurrences、0 unmatched、0 ambiguous；precision `123/123=100%`、candidate recall `195/195=100%`、unsafe `0/46`，但 exclusion leakage 为 `7/47=14.89%`；`check_scan_evaluation_contract.py` 首先报告 `zed-2551721-0254: exclusion fixture leaked as review_required` | 定量证明当前主要矛盾是外部来源 ownership，不是 span、recall 或 unsafe promotion | 未达到零 leakage，按 ADR 0007 停止生成 r3 最终 bundle/audit，转入最小 HIR 修复 |
...
- 恢复方式：实现 ADR 0007 限定的最小 HIR 来源解析和 sink/origin 分离，重新计算 config hash、scan 与 observational 指标；只有 leakage=0 后才生成 r3 blind bundle并启动全量独立审核。
